...
while True:
  #to avoid OverflowError
    ...
    try:
        csv.field_size_limit(maxInteger)
        break
    except OverflowError:
        maxInteger = int(maxInteger/10)
...
def anno_processing(freq_thr=9):
    ...
    train_anno = json.load(open(train_anno_path), encoding = 'utf-8')['annotations']
    val_anno = json.load(open(val_anno_path), encoding = 'utf-8')['annotations']
    anno_s = train_anno + val_anno

    print("Calculating the freq of each multiple choice answers:")
    mca_frequency = {}
    for anno in tqdm(anno_s):
...
def qa_processing(trgt, max_words=14):
    ...
    train_ques = json.load(open(train_ques_path))['questions']
    val_ques = json.load(open(val_ques_path))['questions']
    q_s = train_ques + val_ques
    que_ans_s = []
    for i, q in enumerate(tqdm(q_s)):
    ...
    pickle.dump(que_ans_s[:len(train_ques)], open(os.path.join('data', 'train_qa.pkl'), 'wb'))
    pickle.dump(que_ans_s[len(train_ques):], open(os.path.join('data', 'val_qa.pkl'), 'wb'))
